...
// All lines to check (horizontal rows + diagonals)
const LINES = [
  [1, 2],    // Row 1
  [3, 4, 5], // Row 2
  [0, 1, 3], // Left diagonal
...
const LINE_LABELS = ['[1,2]', '[3,4,5]', '[0,1,3]', '[0,2,5]']
...
export default function MakeNumbers() {
    ...
    set(p => {
      ...
      if (deckEmpty) {
        return {
          ...
          phase: 'over',
          msg: collected > 0
            ? `玩家 ${p.player + 1} 收集了 ${collected} 张牌！游戏结束！`
            : '游戏结束！',
          glow: [...glowCells],
      ...
      return {
        ...
        phase: 'play',
        msg: collected > 0
          ? `玩家 ${p.player + 1} 收集了 ${collected} 张牌！`
          : '',
        glow: [...glowCells],
        showRules: p.showRules,
      }
  ...
  const canPlace = s.phase === 'play' && s.card !== null

  return (
    <div>
      <BackButton />
      <h1 className="text-2xl font-bold text-center mb-1">凑十</h1>
      <p className="text-center text-slate-500 text-xs mb-4">Make Numbers</p>
      ...
      {s.card !== null && (
        <div className="flex justify-center items-center gap-3 mb-4">
          <span className="text-slate-400 text-sm">抽到的牌</span>
          <div className="w-14 h-[4.5rem] bg-gradient-to-br from-amber-50 to-amber-200 border-2 border-amber-600 rounded-lg flex items-center justify-center text-3xl font-bold text-amber-900 shadow-lg shadow-amber-900/20">
            {s.card}
          </div>
